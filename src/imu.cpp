...
void IMU::testImu(std::string src, std::string dist)
{
    ...
    // TODO: mean value intergral
    // store first imu data
    tmp_imupose = imudata[0];
    for (int i = 1; i < imudata.size(); ++i) {
        ...
        /// 中值积分
        // update quaternion to current time step
        Eigen::Quaterniond curr_Qwb;
        curr_Qwb = Qwb * dq;
        // use last and current quaternion to calculate acc.
        Eigen::Vector3d acc_w = 0.5 * (Qwb * (tmp_imupose.imu_acc) + curr_Qwb * (imupose.imu_acc)) + gw; // aw = 0.5( Rwb*(acc_body-acc_bias)+gw + nextRwb*(next_acc_body-acc_bias)+gw)
        Vw = Vw + acc_w * dt;
        Pwb = Pwb + Vw * dt + 0.5 * dt * dt * acc_w;
        // store current quaternion & imu data
        Qwb = curr_Qwb;
        tmp_imupose = imupose;
